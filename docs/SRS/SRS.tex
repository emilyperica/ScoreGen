...
26/03/2025 & 0.4 & \href{https://github.com/emilyperica/ScoreGen/issues/309}{Issue \#309}\\
\bottomrule
\end{tabularx}
...
\subsubsection*{Core Signal Processing Algorithms/Mathematical Framework}
\textbf{Description}: Signal processing algorithms are essential for analyzing audio input and extracting meaningful data for transcription. A mathematical framework for handling audio signals, such as Fourier transforms, spectral analysis, and pitch detection, is crucial for accurate transcription.\\
\textbf{Rationale for Limited Use}: Developing our signal processing algorithms will provide valuable learning opportunities and insights into the complexities of audio analysis. While existing libraries and tools offer robust signal processing capabilities, building these algorithms from scratch will deepen our understanding of the underlying principles.\\
\newline
The following equations define the three major parts of the standard signal processing algorithms that will likely be used in the project:
\begin{enumerate}
  \item \textbf{The Input Signal:}
  Below is a generic definition of a signal to provide an example. 
  \begin{framed}
    \noindent Let the following define an input signal $x$ as a function: 
    \begin{equation}
      x : \mathbb{Z} \to \mathbb{R}
    \end{equation}
    where the signal has finite energy, i.e.,
    \begin{equation}
      E = \sum_{n=-\infty}^{\infty} |x[n]|^2 < \infty
    \end{equation}
  \end{framed}
  \item \textbf{The Window Function}: A function that is zero-valued outside of a specific interval, to be used for the reduction of spectral leakage. The windowing
  function $w(n)$ localizes the effect of the Fourier transform to a finite segment/frame. It allows the capturing of the evolution of frequency content over a specfic time range ~\cite{windows-harmonic-analysis}.
  \begin{framed}
    \noindent For a window length $N\in\mathbb{Z}^{+}$ and $N\geq1$, the Hanning window is defined over: 
    \begin{equation}
      w : \{0, 1, ..., N-1\} \to \mathbb{R}
    \end{equation}
    ...
    \]
  \end{framed}
  \item \textbf{The Short-Time Fourier Transform}: A mathematical transformation that decomposes a function of time into its constituent frequencies.
  The use of the window function lends itself to processing the signal in overlapping segments/frames ~\cite{onset-detection, ZhouRiess}.\\
  \begin{framed}
    Let the hop size be $H\in\mathbb{Z}^{+}$. For each frame at index $m$, we can get a frame of the signal:
    \begin{equation}
      \{x [ mH + n ] : n = 0, 1, ..., N-1\}
    \end{equation}
    The Fourier transform of the windowed frame is defined as:
    \begin{equation}
      STFT\{ x \} : \mathbb{Z} \times \{n = 0, 1, ..., N-1\} \to \mathbb{C}
    \end{equation}
    and is given by:
    \begin{equation}
    ...
    \end{equation}
    where \\
    $m \in \mathbb{Z}$ is the index of a frame and,\\
    $k \in \{0, 1, ..., N-1\}$ is the index of a particular frequency bin.
  \end{framed}
\end{enumerate}
