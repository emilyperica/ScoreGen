...
\begin{enumerate}
    ...
    \textbf{Ian:} \\
    \textbf{Jackson:} There was one main pain point that sticks out to me for this deliverable which is the generation of PDF files from the LaTeX code. This posed a problem 
                    when we went to commit the PDFs to the repository, as we were prompted to manage merge conflicts with a file that was deemed ‘too complex’. As a result, we created a CI job 
                    to re-generate PDFs based on merges for our LaTeX files, resolving our issue and removing the need to directly commit PDF files.\\
    \textbf{Mark:} \\
