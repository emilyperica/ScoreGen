...
// Custom implementation of __ms_vsnprintf
...
std::string determineNoteType(float noteDuration, int bpm) {
    ...
    float beatDuration = 60.0f / bpm; // Beat duration in seconds
    float beatsPerNote = noteDuration / beatDuration; // Convert to beats

    // Find the closest note using std::min_element
    auto closest = std::min_element(
        note_durations.begin(),
        note_durations.end(),
        [beatsPerNote](const auto& a, const auto& b) {
            return std::abs(a.second - beatsPerNote) < std::abs(b.second - beatsPerNote);
        }
    );
...
float getBufferBPM(const std::vector<float>& buf, int sample_rate, const std::map<std::string, std::string>& params = {}) {
    ...
    auto beatsToBPM = [](const std::vector<float>& beats) {
        if (beats.size() > 1) {
            if (beats.size() < 4) {
...
vector<Note> detectNotes(const vector<float>& buf, int sample_rate, int channels) {
    ...
    // Filter out notes with duration less than 0.1 seconds
    notes.erase(
        std::remove_if(notes.begin(), notes.end(), [](const Note& note) {
            return (note.endTime - note.startTime) < 0.1;
            }),
        notes.end()
    );
